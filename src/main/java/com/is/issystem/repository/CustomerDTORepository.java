...
public interface CustomerDTORepository extends JpaRepository<CustomerDTO,Integer>{
    ...
    @Query(nativeQuery = true,value = "SELECT ci.*,code , status ,conadd_city , conadd_district \n" +
            ...
            "INNER JOIN current_address as curadd ON curadd.curadd_id = ci.id_current_address \n" +
            "INNER JOIN permanent_address as peradd on peradd.peradd_id = ci.id_permanent_address \n" +
            "INNER JOIN workplace_address as workadd ON workadd.workadd_id = ci.id_workplace_address where ci.id = ?1 and ci.code_em_spport = ?2")
    CustomerDTO getOneCustomerInfo(Integer id,String code);
