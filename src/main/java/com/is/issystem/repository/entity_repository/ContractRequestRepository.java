...
public interface ContractRequestRepository extends JpaRepository<ContractRequestDTO,Integer> {
    @Query(nativeQuery = true,value = "SELECT re.id as id_request,re.date as start_time,re.id_contract,re.status,re.description,\n" +
            ...
            "INNER JOIN is_agency_db.customer_info as ci on ci.id = ct.id_customer WHERE re.status = \"CXD\"")
    ...
    @Query(nativeQuery = true,value = "SELECT re.id as id_request,re.date as start_time,re.id_contract,re.status,re.description,\n" +
            ...
            "INNER JOIN is_agency_db.contract as ct on re.id_contract = ct.id\n" +
            "INNER JOIN is_agency_db.employee_info as ei on ct.id_employee = ei.id\n" +
            "INNER JOIN is_agency_db.customer_info as ci on ci.id = ct.id_customer WHERE NOT re.status = \"CXD\"")
    List<ContractRequestDTO> getAllRequestContractApproval();
